...
try:
    with open('outputs/predictions/hybrid_sarimax_gru_preds.pkl', 'rb') as f:
        hybrid_preds = pickle.load(f)
        for k, v in hybrid_preds.items():
            if isinstance(v, dict) and 'test' in v:
                pred_real = v['test']
                model_disp_name = "Hybrid"
                preds_dict[model_disp_name] = pred_real
                m = regression_metrics(y_test_real, pred_real)
                metrics_list.append({'Model': model_disp_name, 'Group': 'Hybrid', **m})
except FileNotFoundError:
    print("Hybrid predictions not found. Run notebook 13 first.")
