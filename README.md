...
### Các bước chi tiết:
1. **Dữ liệu thô và Làm sạch cơ bản:** Gộp dữ liệu từ nhiều nguồn, loại bỏ trùng lặp, sửa kiểu dữ liệu.
2. **Phân tích khám phá (EDA - Train Only):** Sử dụng R và Python cho phân tích chéo biến nhằm tránh Data Leakage.
3. **Tiền xử lý (Preprocessing):** Xử lý khuyết thiếu (Imputation), kỹ thuật đặc trưng (Feature Engineering - Lag/Cyclical), biến đổi Log1p đối với PM2.5, Capping ngoại lệ và Min-Max Scaling.
4. **Huấn luyện mô hình:** Thử nghiệm tập hợp SARIMA, SARIMAX, XGBoost, LightGBM, LSTM, GRU và mô hình kết hợp lai Hybrid SARIMAX-GRU.
5. **Đánh giá & Báo cáo:** Sử dụng metric đo lường tiêu chuẩn (RMSE, MAE, MAPE).
