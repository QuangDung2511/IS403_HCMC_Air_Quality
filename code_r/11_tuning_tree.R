...
# ---------------------------------------------------------
# CẤU HÌNH ĐƯỜNG DẪN & SETUP (động, không hard-code)
# ---------------------------------------------------------
script_dir <- tryCatch(
...
train_df <- read_csv(file.path(in_fs_dir, "train_tree.csv"), show_col_types = FALSE) %>% drop_na(all_of(TARGET))
val_df   <- read_csv(file.path(in_fs_dir, "val_tree.csv"),   show_col_types = FALSE) %>% drop_na(all_of(TARGET))
test_df  <- read_csv(file.path(in_fs_dir, "test_tree.csv"),  show_col_types = FALSE) %>% drop_na(all_of(TARGET))
...
ggsave(file.path(fig_dir, "tuned_tree_test_actual_vs_predicted.png"), plot = p_scatter, width = 12, height = 5)

# ---------------------------------------------------------
# 7. EXPORT KẾT QUẢ
# ---------------------------------------------------------
# Lưu mảng dự báo ra RDS
saveRDS(preds_test, file.path(pred_dir, "tuned_tree_preds.rds"))
...
write_json(best_params_json, file.path(pred_dir, "tuned_tree_best_params.json"), pretty = TRUE)
