...
# TRỌNG TÂM: Lấy dữ liệu nguồn từ thư mục Python đã chạy xong
...
train_tree <- read_csv(paste0(in_fs_dir, "train_tree.csv"), show_col_types = FALSE)
val_tree   <- read_csv(paste0(in_fs_dir, "val_tree.csv"), show_col_types = FALSE)
test_tree  <- read_csv(paste0(in_fs_dir, "test_tree.csv"), show_col_types = FALSE)
...
# Dùng scale() của Base R (không cần thư viện caret)
...
write_csv(train_dl, paste0(out_fs_dir, "train_dl.csv"))
write_csv(val_dl, paste0(out_fs_dir, "val_dl.csv"))
write_csv(test_dl, paste0(out_fs_dir, "test_dl.csv"))
...
saveRDS(scaler_info, paste0(data_dir, "preprocessing_dl_scaler.rds"))
...
write_json(meta, paste0(data_dir, "preprocessing_dl_meta.json"), pretty = TRUE, auto_unbox = TRUE)
