...
# TRỌNG TÂM: Đọc file từ thư mục Python (Không có chữ _R)
...
out_csv  <- paste0(base_dir, "/outputs_R/predictions/tree_model_metrics.csv")
...
train_df <- read_csv(paste0(in_fs_dir, "train_tree.csv"), show_col_types = FALSE)
val_df   <- read_csv(paste0(in_fs_dir, "val_tree.csv"), show_col_types = FALSE)
test_df  <- read_csv(paste0(in_fs_dir, "test_tree.csv"), show_col_types = FALSE)
...
ggsave(paste0(fig_dir, "tree_models_test_metrics_bar.png"), plot = p_bar, width = 10, height = 5)
print(p_bar)
...
ggsave(paste0(fig_dir, "tree_models_test_actual_vs_predicted.png"), plot = p_scatter, width = 14, height = 4.5)
print(p_scatter)
...
ggsave(paste0(fig_dir, "tree_models_test_timeseries_subset.png"), plot = p_ts, width = 12, height = 4)
print(p_ts)
